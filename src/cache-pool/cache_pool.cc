...
int CacheManager::Init() {
    ...
    for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        if (!cache_host || !str_cache_port || !str_cache_db ||
            !str_max_conn_cnt) {
            if(!cache_host)
                LOG_ERROR("not configure cache instance: %s, cache_host is null.", pool_name);
            if(!str_cache_port)
                LOG_ERROR("not configure cache instance: %s, cache_port is null.", pool_name);
            if(!str_cache_db)
                LOG_ERROR("not configure cache instance: %s, cache_db is null.", pool_name);
            if(!str_max_conn_cnt)
                LOG_ERROR("not configure cache instance: %s, max_conn_cnt is null.", pool_name);
            return 2;
        }
